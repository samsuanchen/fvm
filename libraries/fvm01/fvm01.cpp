...
/////////////////////////////////////////////////////////////////////////////////////////////////////
void FVM::error(uint8_t id, char* format, ...){
  PRINTF("\nError %03d ", id);
  va_list arglist;
  va_start( arglist, format );
  Serial.printf( format, arglist );
  va_end( arglist );
  T->error=id, initTib();
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
void FVM::errorMessage(uint8_t id, char *message){
  PRINTF( "\nError %03d %s ", id, message ), T->error=id, initTib(); }
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
boolean FVM::isFloatingNumber( char *tkn ){ // use /-?(\d+|\d+\.\d*|\.\d+)(e-?\d+)?/ to check tkn
  ...
  char *p = tkn, c = *p, d = 0;
//PRINTF("\ncheck if \"%s\" is a floating number ", p);
  if( c == '-' ) WRITE(c), c = *(++p);
  while( c>='0' && c <='9' ) d = c, WRITE(c), c = *(++p);
  if( c == '.' ){ WRITE(c), c = *(++p);
    while( c>='0' && c <='9' ) d = c, WRITE(c), c = *(++p);
  }
...
int	FVM::toNumber(char *token){ int sign = 1, n = 0; // assume the number is positive 0
  ...
  } else n = strtol(remain, &remain, b); // convert to integer on base b (*remain is non-digit)
  return n; // n is invalid if *remain != 0.
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
void FVM::dotR(int v, int n, char c){ // print v as n digits leading c (given char 0 or blank)
  char*s=toDigits(v,T->base); for(int8_t i=strlen(s); i<n; i++) WRITE(c); PRINTF("%s ",s); }
/////////////////////////////////////////////////////////////////////////////////////////////////////
void FVM::dotS(){ // show stack
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
boolean FVM::dFull     ()      { return T->DP   >= T->DS+DS_SIZE; } // check if stack full
...
Task * FVM::createTask( int sizeTib, int sizeDS, int sizeRS, int sizeCS ){
  ...
  if(hint) PRINTF("\ncreate task 0x%x alloc %d-byte ", T, bytesTotal);
...
void FVM::vocInit (Word*wrd) {
  ...
  voc->nWord=0;
  while(w) voc->nWord++, first=w, w=w->link;
  voc->first = first;
  voc->lastId=T->last->id;
#ifdef TRACING
  PRINTF("\n%d forth words predefined, the first is 0x%x \"%s\", the last is 0x%x \"%s\" ",
  ...
  w_doNext  =vocSearch("\x06" "(next)"  ); needExtraCell[0]=i_doNext =(int)w_doNext ->code;
  w_doIf    =vocSearch("\x04" "(if)"    ); needExtraCell[1]=i_zbran  =(int)w_doIf   ->code;
  w_doElse  =vocSearch("\x06" "(else)"  ); needExtraCell[2]=i_bran   =(int)w_doElse ->code;
  w_doLit   =vocSearch("\x05" "(lit)"   ); needExtraCell[3]=i_doLit  =(int)w_doLit  ->code;
  w_compile =vocSearch("\x07" "compile" ); needExtraCell[4]=i_compile=(int)w_compile->code;
#ifdef TRACING
...
void FVM::see(Word *w) { // show the forth word
  if(! isWord(w)){
    ABORT( T->error, 5, initTib, "not forth word to see %x ", w); return; }
...
void    FVM::initTib() { // set to input char from serial port
  ...
  if(hint){
	  ...
	  PRINTF("--------------------////////-------------------\n");
	  showTime(), PRINTF("input %02d ", lineIndex++); // ready to input char from serial port
  } else PRINTF(" OK ");
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
void FVM::init( long baud ){
  Serial.begin( baud );
  PRINTF("==================================================\n");
  PRINTF("            Wifi Boy ESP32 forth System           \n");
  PRINTF("20180514 derek@wifiboy.org & samsuanchen@gmail.com\n");
  PRINTF("==================================================\n");
  T = createTask();
  T->DP = T->DS-1;           // clearing data   stack
  T->RP = T->RS-1;           // clearing return stack
  T->error = 0;
  mountFFS();                // mount flash files system
  vocInit( predefined ), initTib();
}
/////////////////////////////////////////////////////////////////////////////////////////////////////
